function [MSE_Tot, MSE_Tg, MSE_Test, InputWeight,OutputWeight]     = OneLayerBackprop_new_Biased(s_ratio_per_7,gamma,stepSize,Data_Train,Target_Train,NumberofHiddenNeurons,C,maxEpoch,NumberofInputNeurons,Winit,Data_Test,Target_Test)
...
H = ones(NumberofHiddenNeurons+1, NumberofTrainingData);

% H = ones(NumberofHiddenNeurons, NumberofTrainingData); % removing bias of hidden layer, August 01,2012
...
fid1 = fopen(strcat(['Results\accuracy_Regularization_' num2str(C) '_num_' num2str(NumberofHiddenNeurons) '_StepSize_' num2str(stepSize) '.txt']), 'w');
...
%figure ; mesh(OutputWeight)  
    ...
    H_test = ones(NumberofHiddenNeurons+1, NumberofTestingData);    

%     H_test = ones(NumberofHiddenNeurons, NumberofTestingData); % removing bias of hidden layer, August 01,2012
