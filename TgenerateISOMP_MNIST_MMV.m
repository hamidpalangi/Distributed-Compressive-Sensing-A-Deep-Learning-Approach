function [rMM,sMM] = TgenerateISOMP_MNIST_MMV(phi_m,L,DataPath,nImage,Idim,nsparseTrain,Bdim,TransformProps)
...
% Normalization starts******************************************
    for i4=1:size(rMM,3)       
       temp4 = rMM(:,:,i4); 
       rMM(:,:,i4) = rMM(:,:,i4) ./ max( abs( temp4(:) ) );
    end
    for i4=1:size(sMM,3)       
       temp5 = sMM(:,:,i4); 
       sMM(:,:,i4) = sMM(:,:,i4) ./ max( abs( temp5(:) ) );
    end
% Normalization ends******************************************
end